 ...
 * WebFinger as described in RFC 7033 is a spec that defines a method for resolving links to a resource, given only a
 * URI on a particular server. This allows anyone to look up where a resource is located without having to know its
 * exact location beforehand; for example, by email or phone number. This lookup is directed at the endpoint
 * /.well-known/webfinger, and a resource query parameter is passed along with the lookup.
 * The resource URI used with Mastodon is the acct: URI as described in RFC 7565, with the username of a profile that
 * is hosted on a particular domain.
 */
...
public class WebFingerTests : IDisposable
{
    ...
    {
        _log = log;
        _log.WriteLine($"Bogus Seed: {Init.WithSeed()}");
        _profile = new FakeProfile("uss-voyager.example");
        _web = new WebAdapter();
        _client = _web.Client;

        var profile = _profile.Generate();
        // _web.FakeAccountService.AlwaysReturn(Profile.CreatePerson(new Uri("https://uss-voyager.example"), ""));
        // _web._mockAccountService
            // .Setup(m => m.LookupProfile())
    }
